...
6. Run the simulation using the blue play button which performs the steady-state simulations. 
<p align="center">
...
4. Click on **Cut Line Plot** and select appropriate options. In this example, using the correct device file (.sdm), circuit file (.res), orientation of the cut line as vertical, variable to be plotted as Net Doping (Na-Nd) etc. Click on **New Plot**. Shown below are the plots like new doping, energy band diagram etc.
